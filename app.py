...
def delete_appointment(inspection_id, db_path=DB_PATH):
    with sqlite3.connect(db_path) as connection:
        connection.execute(
            "DELETE FROM appointments WHERE inspection_id = ?",
            (inspection_id,),
        )
        connection.execute(
            "DELETE FROM notification_log WHERE inspection_id = ?",
            (inspection_id,),
        )
        connection.commit()
...
def ortools_assign_and_optimize_routes(data, inspectors):
    ...
    if solution is None:
        ...
        raise ValueError(
            ...
            f"time windows, inspection durations, and {routing_mode}. "
...
def render_app():
    ...
    if not valid_dates:
        ...
        st.stop()
    pending_date = st.session_state.pop("pending_inspection_day", None)
    if pending_date in valid_dates:
        st.session_state["inspection_day"] = pending_date
    if st.session_state.get("inspection_day") not in valid_dates:
        st.session_state["inspection_day"] = valid_dates[0]
        ...
        if submitted:
            if not customer_name.strip() or not address.strip():
                ...
                if location is None:
                    ...
                    st.session_state["pending_inspection_day"] = requested_date
                    st.session_state["pending_week_start"] = (
                        requested_date
                        - timedelta(days=requested_date.weekday())
                    )
                    st.rerun()

        online_appointments = read_custom_appointments()
        st.divider()
        st.markdown("**Online appointments**")
        if online_appointments.empty:
            st.info("No online appointments have been created yet.")
        else:
            st.caption(
                "These appointments are stored independently of the currently "
                "selected day and week."
            )
            st.dataframe(
                online_appointments[
                    [
                        "inspection_date", "inspection_id", "customer_name",
                        "site_address", "inspection_type", "priority", "status",
                        "inspector_name",
                    ]
                ],
                use_container_width=True,
            ...
            st.markdown("**Delete an online appointment**")
            delete_id = st.selectbox(
                "Choose appointment to delete",
                online_appointments["inspection_id"].tolist(),
                format_func=lambda value: (
                    f"{value} · "
                    f"{online_appointments.loc[online_appointments['inspection_id'] == value, 'customer_name'].iloc[0]}"
                ),
                key="delete_appointment_id",
            )
            confirm_delete = st.checkbox(
                "I confirm that this appointment may be permanently deleted",
                key="confirm_delete_appointment",
            )
            if st.button(
                "Delete appointment",
                type="primary",
                ...
                if st.session_state.get("inspection_day") == (
                    online_appointments.loc[
                        online_appointments["inspection_id"] == delete_id,
                        "inspection_date",
                    ].iloc[0]
                ):
                    st.session_state.pop("inspection_day", None)
                st.session_state.pop("route_selection_key", None)
                st.session_state["appointment_deleted_message"] = (
                    f"Appointment {delete_id} was deleted."
                )
                st.rerun()
        if "appointment_deleted_message" in st.session_state:
            st.success(st.session_state.pop("appointment_deleted_message"))

    with week_tab:
        st.subheader("Weekly capacity and planning proposal")
        default_monday = selected_date - timedelta(days=selected_date.weekday())
        pending_week_start = st.session_state.pop("pending_week_start", None)
        if pending_week_start is not None:
            st.session_state["week_start"] = pending_week_start
        elif "week_start" not in st.session_state:
            st.session_state["week_start"] = default_monday
        week_start = st.date_input(
            "Week starting", key="week_start"
        )
        week_end = week_start + timedelta(days=6)
    ...
    with routes_tab:
        ...
        if st.session_state.routes_optimized:
            ...
            st.caption(
                ...
                "configured inspection durations are enforced."
            )
        else:
            st.info(
